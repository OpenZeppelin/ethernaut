...
contract Statistics is Initializable {
    ...
    function getTotalNoOfFailuresForLevelAndPlayer(address level, address player) 
        ...
        public view returns(uint256) {
            return  playerStats[player][level].instance != address(0) ?
            playerStats[player][level].timeSubmitted.length : 0;
    }
        ...
        returns (uint256)
    {
        ...
        return playerStats[player][level].timeCompleted - playerStats[player][level].timeCreated;
    }
        ...
        returns (uint256)
    {
        ...
        return
            (getTotalNoOfLevelsCompletedByPlayer(player) * 1e18) / levels.length;
    }
