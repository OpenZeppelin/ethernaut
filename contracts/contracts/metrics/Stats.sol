...
contract Stats {
    ...
    function saveCreateLevelStats(
        address levelInstanceAddress,
        ...
        require(
            playerStats[msg.sender][levelFactoryAddress].instanceAddress ==
                address(0),
            "Level already created"
        );
        playerStats[msg.sender][levelFactoryAddress] = LevelInstance(
            levelInstanceAddress,
            false,
            block.timestamp,
            0,
            new uint256[](0)
    ...
    function saveSubmitLevelStats(address levelFactoryAddress, bool isCompleted)
        public
    {
        require(
            playerStats[msg.sender][levelFactoryAddress].instanceAddress !=
        ...
        require(
            playerStats[msg.sender][levelFactoryAddress].isCompleted == false,
            "Level already completed"
        );

        playerStats[msg.sender][levelFactoryAddress].timeSubmitted.push(
            block.timestamp
        );

        if (isCompleted) {
            levelStats[levelFactoryAddress].noOfInstancesSubmitted_Success++;
            playerStats[msg.sender][levelFactoryAddress].timeCompleted = block
                .timestamp;
