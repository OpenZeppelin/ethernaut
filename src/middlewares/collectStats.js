...
export default store => next => action => {
  if (action.type !== actions.COLLECT_STATS) {
    if (
      queuedAction &&
      action.type === actions.LOAD_ETHERNAUT_CONTRACT &&
      action.contract
    ) {
  ...
  if (!action.createdInstanceLogs) {
    const instanceCreatedLog = state.contracts.ethernaut.methods.LevelInstanceCreatedLog(
      query.filter,
      query.range
    );
  ...
  if (!action.completedLevelLogs) {
    const levelCompletedLog = state.contracts.ethernaut.methods.LevelCompletedLog(
      query.filter,
      query.range
    );
